...
const (
	strType fmtType = 1 + iota
	formatType
)

var cache = make(map[string]pp)
...
func Sprintf2(format string, a ...any) string {
	if p, ok := cache[format]; ok {
		...
		for _, node := range p.nodes {
			...
			case formatType:
				p.fmt = node.fmt
				p.printArg(a[node.argIdx], node.verb)
				l += len(node.format)
			default:
				panic(stdfmt.Errorf("errors nodes:%+v", node))
			}
		}
		for _, node := range p.nodes {
			switch node.typ {
			case strType:
				p.buf.writeString(node.format)
			case formatType:
				p.fmt = node.fmt
				p.printArg(a[node.argIdx], node.verb)
			default:
...
formatLoop:
	for i := 0; i < end; {
		...
		if i > lasti {
			...
			p.nodes = append(p.nodes, fmtNode{typ: strType, format: format[lasti:i]})
		}
		if i >= end {
		...
		for ; i < end; i++ {
			...
			default:
				...
				if 'a' <= c && c <= 'z' && argNum < len(a) {
					...
					p.nodes = append(p.nodes, fmtNode{typ: formatType, fmt: p.fmt, argIdx: argNum, verb: rune(c)})
